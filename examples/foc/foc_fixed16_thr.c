...
#ifndef CONFIG_POWER_FOC_USE_FIXED16
#  error
#endif
...
struct foc_motor_b16_s
{
  ...
  b16_t         vbus;           /* Motor VBUS */
  b16_t         angle_now;      /* Motor angle now */
  b16_t         vel_set;        /* Motor velocity set */
  b16_t         vel_now;        /* Velocity now */
  b16_t         vel_des;        /* Current velocity destination */
  b16_t         dir;            /* Speed direction */
                          ...
                          FAR struct foc_ctrl_cfg_s *ctrl)
{
  ...
  PRINTFV("  id_kp: %.3f\n", b16tof(ctrl->id_ki.b16));
  PRINTFV("  id_ki: %.3f\n", b16tof(ctrl->id_ki.b16));
  PRINTFV("  iq_kp: %.3f\n", b16tof(ctrl->iq_ki.b16));
  PRINTFV("  iq_ki: %.3f\n", b16tof(ctrl->iq_ki.b16));
...
static int foc_cfg_configure(int fd, uint32_t kp, uint32_t ki)
{
  ...
  ctrl.id_kp.b16 = b16idiv(kp, 1000);
  ctrl.id_ki.b16 = b16idiv(ki, 1000);
  ctrl.iq_kp.b16 = b16idiv(kp, 1000);
  ctrl.iq_ki.b16 = b16idiv(ki, 1000);
  ...
  if (ctrl.id_kp.b16 == 0 && ctrl.id_ki.b16 == 0 &&
      ctrl.iq_kp.b16 == 0 && ctrl.iq_ki.b16 == 0)
    {
      PRINTF("ERROR: invalid FOC fixed16 configuration !\n");
      ret = -EINVAL;
      goto errout;
    }
  ...
  if (ret != OK)
    {
      PRINTF("ERROR: foc_fixed16 PWRIOC_SET_CONFIG cmn failed %d!\n", errno);
      ret = -errno;
      goto errout;
    }
  ...
  if (ret != OK)
    {
      PRINTF("ERROR: foc_fixed16 PWRIOC_SET_CONFIG ctrl failed %d!\n",
             errno);
      ret = -errno;
      goto errout;
    }
...
static int focmodel_cfg_configure(int fd)
{
  ...
  cfg.res.b16      = ftob16(FOC_MODEL_RES);
  cfg.ind.b16      = ftob16(FOC_MODEL_IND);
  cfg.interia.b16  = ftob16(FOC_MODEL_J);
  cfg.fluxlink.b16 = ftob16(FOC_MODEL_FLUXLINK);
  cfg.ind_d.b16    = ftob16(FOC_MODEL_LD);
  cfg.ind_q.b16    = ftob16(FOC_MODEL_LQ);
  ...
  params.load.b16 = FOC_MODEL_LOAD;
...
static int foc_mq_handle(mqd_t mq, FAR struct foc_handle_b16_s *h)
{
  int      ret = OK;
  uint8_t  buffer[5];
  ...
  switch (buffer[0])
    {
      ...
      default:
        {
          PRINTF("Invalid foc_fixed16_thr message type %d\n", buffer[0]);
          ret = -EINVAL;
          goto errout;
        }
    }
...
static int foc_motor_initialize(FAR struct foc_motor_b16_s *motor)
{
  int ret = OK;

  /* Initialize motor data */

  motor->ramp_thr = ftob16(RAMP_CFG_THR);
  motor->ramp_acc = ftob16(RAMP_CFG_ACC);
  motor->ramp_dec = ftob16(RAMP_CFG_ACC);
  motor->ramp_per = b16divi(b16ONE, CONFIG_EXAMPLES_FOC_NOTIFIER_FREQ);

  /* Helpers */

  motor->ramp_acc_per = b16mulb16(motor->ramp_acc, motor->ramp_per);
  motor->ramp_dec_per = b16mulb16(motor->ramp_dec, motor->ramp_per);
...
static int foc_motor_vel(FAR struct foc_motor_b16_s *motor, uint32_t vel)
{
   ...
   * NOTE: velmax may not fit in b16_t so we cant use b16idiv()
   */

  tmp1 = itob16(motor->velmax / 1000);
...
static int foc_motor_state(FAR struct foc_motor_b16_s *motor, int state)
{
  ...
  motor->dq_ref.b16.q = b16idiv(motor->qparam, 1000);
  motor->dq_ref.b16.d = 0;
...
static int foc_ramp(FAR struct foc_motor_b16_s *motor)
{
  b16_t vel_diff;
                         ...
                         FAR struct foc_params_s *params)
{
  ...
  tmp = b16mulb16(motor->dir, motor->vel_set);
  phase_step = b16mulb16(tmp, motor->ramp_per);

  /* Update open-loop angle */

  motor->angle_now = motor->angle_now + phase_step;

  /* Normalize the open-loop angle to 0.0 - 2PI range */

  angle_norm_2pi_b16(&motor->angle_now,
                     MOTOR_ANGLE_E_MIN_B16, MOTOR_ANGLE_E_MAX_B16);
  ...
  if (fault == false)
    {
      params->dq_ref.b16.q   = motor->dq_ref.b16.q;
      params->dq_ref.b16.d   = motor->dq_ref.b16.d;
      params->angle.b16      = motor->angle_now;
      params->vbus.b16       = motor->vbus;
      params->vdq_comp.b16.d = 0;
      params->vdq_comp.b16.q = 0;
    }
  else
    {
      /* Stop motor */

      params->dq_ref.b16.q   = 0;
      params->dq_ref.b16.d   = 0;
      params->angle.b16      = 0;
      params->vbus.b16       = 0;
      params->vdq_comp.b16.d = 0;
      params->vdq_comp.b16.q = 0;
                    ...
                    mqd_t mqd)
{
  ...
  struct foc_handle_b16_s handle;
  int                     time      = 0;
  int                     ret       = OK;
  ...
  memset(&handle, 0, sizeof(struct foc_handle_b16_s));
  ...
  PRINTF("Set mode=%d for FOC driver %d!\n", motor.foc_mode, envp->id);

  ret = ioctl(dev->fd, PWRIOC_SET_MODE, (unsigned long)&motor.foc_mode);
  if (ret != OK)
    {
      PRINTF("ERROR: foc_fixed16 PWRIOC_SET_MODE failed %d!\n", errno);
      goto errout;
    }
...
errout:
  ...
  if (ret != OK)
    {
      PRINTF("ERROR: foc_fixed16 PWRIOC_STOP failed %d!\n", ret);
    }
